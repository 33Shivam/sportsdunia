import { cn } from "@/lib/utils";
import {
  IconBrandReact,
  IconBrandNextjs,
  IconLetterJ,
  IconBrandTypescript,
  IconContract,
  IconBrandPython,
  IconBrandGithub,
  IconHeart,
} from "@tabler/icons-react";

export function FeaturesSectionDemo() {
  const features = [
    {
      title: "ReactJs",
      description:
        "I turn to ReactJS whenever I need to whip up something special for my frontend.",
      icon: <IconBrandReact />,
    },
    {
      title: "NextJs",
      description:
        "Though I love react, I realy on NextJs whenever backend starts to come.",
      icon: <IconBrandNextjs />,
    },
    {
      title: "Java",
      description:
        "Java is my core programming langauge. I mean you can't go wrong with the lagauge you learnt Data-Structures in ",
      icon: <IconLetterJ />,
    },
    {
      title: "Typescript",
      description: "Due to FOMO I was forced to shift to typescript",
      icon: <IconBrandTypescript />,
    },
    {
      title: "Solidity",
      description:
        "You are into blockchain, Smart Contracts then Solidity is a must.",
      icon: <IconContract />,
    },
    {
      title: "Python",
      description:
        "My go to scripting langauge whenever I need automate stuff. I once booked IRCTC ticket using python.",
      icon: <IconBrandPython />,
    },
    {
      title: "Version Control",
      description:
        "For a coder like myself whose has a history of writing code that is like a wild circus. Git is my go to.",
      icon: <IconBrandGithub />,
    },
    {
      title: "And everything else",
      description:
        "I already punching above my stack. Accept my sincere apologies",
      icon: <IconHeart />,
    },
  ];
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4  relative z-10 py-10 max-w-7xl mx-auto">
      {features.map((feature, index) => (
        <Feature key={feature.title} {...feature} index={index} />
      ))}
    </div>
  );
}

export default FeaturesSectionDemo;

const Feature = ({
  title,
  description,
  icon,
  index,
}: {
  title: string;
  description: string;
  icon: React.ReactNode;
  index: number;
}) => {
  return (
    <div
      className={cn(
        "flex flex-col lg:border-r  py-10 relative group/feature dark:border-neutral-800",
        (index === 0 || index === 4) && "lg:border-l dark:border-neutral-800",
        index < 4 && "lg:border-b dark:border-neutral-800"
      )}
    >
      {index < 4 && (
        <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-t from-neutral-100 dark:from-neutral-800 to-transparent pointer-events-none" />
      )}
      {index >= 4 && (
        <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-b from-neutral-100 dark:from-neutral-800 to-transparent pointer-events-none" />
      )}
      <div className="mb-4 relative z-10 px-10 text-neutral-600 dark:text-neutral-400">
        {icon}
      </div>
      <div className="text-lg font-bold mb-2 relative z-10 px-10">
        <div className="absolute left-0 inset-y-0 h-6 group-hover/feature:h-8 w-1 rounded-tr-full rounded-br-full bg-neutral-300 dark:bg-neutral-700 group-hover/feature:bg-blue-500 transition-all duration-200 origin-center" />
        <span className="group-hover/feature:translate-x-2 transition duration-200 inline-block text-neutral-800 dark:text-neutral-100">
          {title}
        </span>
      </div>
      <p className="text-sm text-neutral-600 dark:text-neutral-300 max-w-xs relative z-10 px-10">
        {description}
      </p>
    </div>
  );
};
